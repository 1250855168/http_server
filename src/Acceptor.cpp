...
Acceptor::Acceptor(std::shared_ptr<EventLoop> _loop)
  ...
  sock = new Socket();
  int flag = 1;
  setsockopt(sock->getFd(), SOL_SOCKET, SO_REUSEADDR, &flag, sizeof(flag)); // 设置套接字选项，允许地址重用
  InetAddress *addr = new InetAddress("127.0.0.1", 1234); // 创建一个 IPv4 地址对象，监听本地地址和端口 1234
  sock->bind(addr); // 将套接字绑定到指定的地址和端口
  sock->listen(); // 开始监听连接请求
  acceptChannel = new Channel(loop, sock->getFd()); // 创建一个 Channel 对象来处理套接字上的事件
  std::function<void()> cb = std::bind(&Acceptor::acceptConnection, this); // 创建一个回调函数，用于处理可读事件，即接受新连接
...
void Acceptor::acceptConnection() {
  InetAddress *clnt_addr = new InetAddress(); // 创建一个地址对象，用于存储客户端的地址信息
  Socket *clnt_sock = new Socket(sock->accept(clnt_addr)); // 接受客户端的连接请求，返回一个新的套接字对象
  char buf[1024];
  std::sprintf(buf, "new client fd %d! IP: %s Port: %d\n", clnt_sock->getFd(),
               inet_ntoa(clnt_addr->getAddr().sin_addr),
               ntohs(clnt_addr->getAddr().sin_port)); // 打印客户端的文件描述符、IP地址和端口号信息
